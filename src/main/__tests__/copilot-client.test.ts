...
describe("CopilotManager", () => {
  ...
  it("transitions to connected after start", () => {
    const manager = createCopilotManager();
    manager.start();
  ...
  it("passes cliPath when provided", () => {
  ...
  it("passes undefined when no cliPath", () => {
    const manager = createCopilotManager();
    manager.start();
    expect(MockCopilotClient).toHaveBeenCalledWith(undefined);
  });
  ...
  it("sets status to disconnected when constructor throws", () => {
