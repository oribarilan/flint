...
import { filterModels, handleSetModel, type RawModelInfo, type SetModelDeps } from '../ipc/model-handlers'
...
describe('handleSetModel', () => {
  function createMockDeps(
    overrides: Partial<SetModelDeps> = {},
  ): SetModelDeps & { mockSetModel: ReturnType<typeof vi.fn>; mockUpdate: ReturnType<typeof vi.fn>; mockSend: ReturnType<typeof vi.fn> } {
    const mockSetModel = vi.fn().mockResolvedValue(undefined)
    const mockUpdate = vi.fn()
    const mockSend = vi.fn()
    return {
      session: { setModel: mockSetModel },
      configStore: { getAll: vi.fn(), update: mockUpdate } as unknown as SetModelDeps['configStore'],
  ...
  it('calls setModel before persisting (order matters)', async () => {
    ...
    const mockSetModel = vi.fn().mockImplementation(async () => {
