...
function createMockConfig(overrides: Partial<FlintConfig> = {}): FlintConfig {
  return {
...
describe("PulseScheduler", () => {
  ...
  it("subsequent poll includes lastPollTime", async () => {
    ...
    const secondCall = mocks.sendMonitorPoll.mock.calls[1][0] as {
      lastPollTime?: string;
  ...
  it("poll failure is logged and next poll is scheduled", async () => {
    ...
    expect(mocks.sendMonitorPoll).toHaveBeenCalledTimes(1);
    expect(console.error).toHaveBeenCalledWith("[pulse] Poll failed:", "network error");
