...
function buildContextPrefix(selectedItems: AttentionItem[]): string {
  ...
  const lines = selectedItems.map((item) => {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- metadata may be missing from AI-generated items
...
export function useChat() {
  ...
  useEffect(() => {
    const unsubDelta = window.flint?.onChatDelta((delta) => {
      appendDelta(delta);
    });
    const unsubDone = window.flint?.onChatDone(() => {
      finishStreaming();
    });
    return () => {
      unsubDelta?.();
