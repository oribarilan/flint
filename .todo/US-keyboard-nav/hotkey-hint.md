...
- [ ] `HotkeyHint` component accepts a `keys` prop: `string[]` where each string is a key name
- [ ] Modifier keys render as symbols: `ctrl` → `⌃`, `cmd`/`meta` → `⌘`, `shift` → `⇧`, `alt`/`option` → `⌥`
- [ ] Special keys render as symbols: `enter` → `↵`, `space` → `␣`, `escape` → `esc`
- [ ] Regular keys render uppercase: `j` → `J`, `/` → `/`
- [ ] Each key rendered in its own pill/cap: `--bg-secondary` background, `--border-subtle` border, `--radius-sm` corners, `--font-xs` size, `--font-mono` font
- [ ] Text color: `--text-placeholder` (muted, non-competing)
- [ ] Multiple keys render side by side with `--space-xs` gap
- [ ] Component accepts optional `className` prop for positioning by consumers
- [ ] `aria-hidden="true"` on the component (hints are decorative, not functional)
- [ ] Unit tests: renders single key, renders modifier+key combo, renders special keys, applies custom className
