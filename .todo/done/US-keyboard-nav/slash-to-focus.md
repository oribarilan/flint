...
- [x] Pressing `/` when no text input has focus calls `focus()` on the chat input and prevents the `/` from being typed
- [x] Pressing `/` when the chat input already has focus types a literal `/` (no interception)
- [x] Pressing `/` when any other input/textarea/contenteditable has focus types a literal `/` (no interception)
- [x] The chat input shows a styled `<kbd>` element inside the input as the `/` hint (changed from placeholder text per implementation)
- [x] The `/` handler is added to App.tsx's existing `keydown` useEffect, after the escape stack and before the pass-through
- [x] Chat input exposes a ref or imperative handle so App.tsx can call `focus()` on it
- [x] Unit tests: `/` focuses chat input when nothing focused, `/` is ignored when input already focused, hint element present
