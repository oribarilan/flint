...
| **Agent** | Quick AI conversation | Enter sends message |

### Mode Switching

- **Tab** toggles between search and agent mode at any time.
- The current mode is visually indicated by:
  - **Icon swap** in the search bar (magnifying glass ↔ sparkle/agent icon).
...
- Multi-turn: follow-up messages continue the same agent session.
...
- An agent session **persists across window hide/show** (hotkey dismiss + re-open preserves it).
- An agent session is **cleared by Escape** (see layering below) — there is no disk persistence.
...
2. **Agent session active** (messages exist) → clear the agent session, return to search mode.
...
| **Agent** | AI provider connections & model preferences | Provider auth (OpenCode), default model |
...
#### Agent Settings Detail

- **Provider section**: OpenCode shown as a provider card with connection status. When connected, the model selector appears within the card. When disconnected, a "Connect" button guides the user through provider auth setup.
- **Provider auth**: OpenCode manages provider credentials locally (`~/.local/share/opencode/auth.json`). The Settings UI surfaces connection status and links to provider configuration.
- **Model selection**: Dropdown to choose the default model (e.g., gpt-4.1, claude-sonnet-4). Populated from OpenCode's available model list.
...
- **Full agent experience**: A separate, persistent window (like the Settings window) with conversation history, markdown rendering, code blocks, and copy buttons. Opened from tray menu or a "Continue in full agent →" link. Lowest priority.
- **AI commands with confirmation UI**: Action cards with "Run" button for destructive or system-level operations. Can evolve naturally from the agent flow when tool-calling is added.
- **Intent auto-detection**: Potentially auto-switch to agent mode when the query looks like a question or command (heuristic or LLM-based). For now, Tab toggle is sufficient.
