...
An AI-native application launcher, built to feel fast and invisible. Flint connects to OpenCode for multi-provider AI — just configure your provider and go.
...
- ✅ OpenCode integration (local server, multi-provider)
- ✅ Agent streaming (SSE via reqwest, Tauri events)
...
**Frontend**:
1. **Tab toggle**: Add mode state to a new `appStore` (or extend searchStore). Tab key switches between `search` and `agent`. Remove `/ai` prefix detection.
2. **Visual indicator**: Search bar icon swaps (magnifying glass ↔ sparkle). Subtle background color change on the search bar wrapper in agent mode.
3. **Escape layering**: Refactor Escape handling in App.tsx:
   - Input has text → clear input
   - Agent session active → `clearChat()`, switch to search mode
   - Empty search mode → `hideWindow()`
4. **Agent session persistence**: Don't clear agent session on window hide/show. Only clear on Escape layer 2.
...
1. **Config module** (`src-tauri/src/config.rs`):
   ...
   - Sections: `GeneralConfig`, `SearchConfig`, `AgentConfig`
...
**Frontend**:
1. **Settings layout**: Sidebar (General / Agent / Search) + content panel
2. **General page**: Launch at login toggle, hotkey display (read-only for now)
3. **Agent page**: Provider auth status (Connected/Not connected), sign in/out buttons, model dropdown
...
Fix remaining agent mode issues and improve quality.
...
4. **Clear visual**: "New session" indicator when session is reset.
...
- **Full agent window**: Separate persistent window with markdown rendering, code blocks, copy
